...
export default function MyCoursesCard(myCourseCard: MyCourseCard) {
  return (
    <div className="card w-[249px] h-[304px] bg-white drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)] rounded-[20px]">
      <img
        ...
        src={myCourseCard.courseThumbnailUrl}
        alt="Course Thumbnail Picture"
      />
      <div className="px-3 pt-2 text-[14px] font-bold">
        {myCourseCard.courseName}
      </div>
      <div className="px-3 pt-1 text-[12px] text-gray-400 font-semibold col">
        {myCourseCard.instructorName}
      </div>
      <div className="px-3 pt-3 text-[12px] font-semibold">{`สำเร็จไปแล้ว ${myCourseCard.percentCompleted}%`}</div>
      <div className="px-3">
        <progress
          className="progress"
          value={`${myCourseCard.percentCompleted}`}
          max="100"
        ></progress>
      </div>
