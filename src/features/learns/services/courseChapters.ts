...
async function getCourseChapter(
  ...
  const url = `${BASE_URL}/programs/courses/${courseID}/chapters/${chapterID}`;
...
async function listCourseChapters(courseID: string): Promise<CourseChapter[]> {
  ...
  for (let i = 0; i < res.data.chapters.length; i++) {
    const chapter_url = `${BASE_URL}/programs/courses/${courseID}/chapters/${res.data.chapters[i].chapter_id}`;
