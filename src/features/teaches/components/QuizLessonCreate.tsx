...
function _ProblemCreate({
  ...
  const [correctAnswer, setCorrectAnswer] = useState<
    CourseQuizProblem["correctAnswers"]
  >({
    choiceA: false,
    choiceB: false,
    choiceC: false,
  ...
  const handleSubmit = () => {
    ...
    if (onSubmit) {
      ...
      const newProblem: CourseQuizProblem = {
        problemNumber: problemToBeEdit.problemNumber,
        ...problem,
      };
      onEdit(newProblem);
    }
  ...
  useEffect(() => {
    if (problemToBeEdit) {
      ...
      for (let i = 0; i < Object.keys(problemToBeEdit.choices).length; i++) {
        if (
          problemToBeEdit.choices[Object.keys(problemToBeEdit.choices)[i]] ===
          ""
        ) {
          setNumChoice(i);
          break;
  ...
  return (
    <div className="flex flex-col ml-[70px] mr-[100px] mt-[30px] bg-white drop-shadow-xl">
      ...
      <h1 className="my-auto mx-[40px] font-semibold text-[15px] text-black">
        ติ๊กถูกเพื่อเลือกข้อที่เป็นจริง
      </h1>
      {Object.entries(choices).map(([key, value], index) => {
        ...
        return (
          <div className="mx-[55px]" key={`${key}-${index}`}>
            <input
              className="mx-3"
              type="checkbox"
            ...
            />
            <input
              className="mr-[50px] my-[10px] h-[25px] min-w-0 grow input input-bordered "
              type="text"
      ...
      <div className="flex">
        {numChoice <= 5 ? (
          <button
            className="mx-14 mb-4 px-2 py-1 flex flex-row items-center bg-gray-100"
            onClick={() => {
              setNumChoice((p) => p + 1);
            }}
          >
      ...
      <button className="btn bg-[#d9d9d9]" onClick={handleSubmit}>
        <h1>ตกลง</h1>
      </button>
    </div>
  );
...
function QuizLessonCreate({
  ...
  function handleLessonDescriptionChange(
  ...
  const onProblemSubmit = (problem: CourseQuizProblemWithoutNumber) => {
    const newProblem: CourseQuizProblem = {
      problemNumber: quiz.problems.length + 1,
      ...problem,
    };
  ...
  const onProblemDelete = (problemNumber: number) => {
    ...
    const newProblems = quiz.problems.filter(
      (p) => p.problemNumber !== problemNumber
    );
  ...
  const handleSubmit = () => {
    ...
    if (quiz.name === "") {
      errorMessage +=
        "<span class='text-red-500 font-medium text-lg'>กรุณากรอกหัวข้อคำถาม</span>";
      errorMessage += "<br>";
    }

    if (quiz.description === "") {
      errorMessage +=
        "<span class='text-red-500 font-medium text-lg'>กรุณากรอกคำอธิบาย</span>";
      errorMessage += "<br>";
    }

    if (quiz.timeLimit === 0) {
      errorMessage +=
        "<span class='text-red-500 font-medium text-lg'>กรุณากรอกเวลาที่ใช้</span>";
      errorMessage += "<br>";
    }
  ...
  return (
    <div className="w-full">
      ...
      {quiz.problems.map((problem, index) => {
        return (
          <div
            className="ml-[70px] mr-[100px] mt-[30px]  bg-white drop-shadow-xl"
            key={index}
          >
            <div className="py-2 bg-[#d0d0d0] flex justify-between">
              <h1 className="my-auto mx-[40px] font-semibold text-[18px]">
                คำถามที่ {problem.problemNumber} {problem.question}
              </h1>
              <div className="flex">
                <button
                  className="mr-5"
                ...
                <button
                  className="mr-5"
                  onClick={() => {
                    onProblemDelete(problem.problemNumber);
                  }}
                >
                  <FontAwesomeIcon icon={faX} size="lg" />
                </button>
            ...
            {Object.entries(problem.choices).map(([key, value], index) => {
              if (value === "") {
                ...
                return (
                  <div
                    className="mx-[55px] my-2 flex items-center"
                    key={`${key}-${value}-${index}`}
                  >
                    <input
                      type="checkbox"
                      checked={problem.correctAnswers[key]}
                      className="mr-5"
                    ></input>
                    <h1 className=" font-semibold text-[16px] text-[#3a3a3a]">
                      {value}
                    </h1>
                  </div>
                );
            ...
            <hr />
            <h1 className=" mx-[40px] py-[10px] font-semibold text-[18px] text-[#3a3a3a]">
              คำอธิบาย :
      ...
      {addMode != "none" ? (
        addMode === "adding" ? (
          ...
          <_ProblemCreate
            ...
            problemToBeEdit={problemToBeEdit}
          />
        )
      ) : (
        <></>
      ...
      <div>
        <button
          className="btn ml-[70px] my-5 bg-[#d9d9d9]"
